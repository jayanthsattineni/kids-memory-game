...
def main(name):
    ...
    print (a,b,c,d)
    root123.title(name)
    c1=Canvas(root123, bg='cyan4',height=600, width=900)
    id=c1.create_rectangle(50,40,850,560, width=2, fill='cadetblue3')
    ...
    for i in range(10):
        if i is not a and i is not b and i is not c and i is not d:
            ...
            if i == 6:
                count+=1
                coord()
                c1.create_image(x1,y1,anchor=NE, image=file6, activeimage=file7)
            if i == 7:
                count+=1
                coord()
